...
const trackableManager = new TrackableManager();

class TrackedSubject {
    constructor(initialValue, compare) {
...
class TrackedComputedSubject {
    ...
    updateSubscriptions(dependencies) {
...
const getTrackedPropertyDefinitionList = (obj) => (obj[TrackedPropertyDefinitionListKey] = obj[TrackedPropertyDefinitionListKey] || {});
const registerTrackedPropertyDefinition = (proto, name, computed /*, initialValue: T[K]*/) => {
...
const getComputedPropertyList = (obj) => (obj[ComputedPropertyListKey] = obj[ComputedPropertyListKey] || {});
const getOrSetupComputedProperty = (obj, name, getter) => {
    const list = getComputedPropertyList(obj);
    const existing = list[name];
    if (!existing) {
        const subject = new TrackedComputedSubject(getter.bind(obj));
        list[name] = { subject };
    }
    return list[name].subject;
};
const getComputedProperty = (obj, name) => {
    const list = getComputedPropertyList(obj);
    if (!list[name]) {
...
const getTrackedPropertyList = (obj) => (obj[TrackedPropertyListKey] = obj[TrackedPropertyListKey] || {});
const getTrackedProperty = (obj, name, notFound) => {
...
const getOrSetupTrackedProperty = (obj, name, value) => {
    return getTrackedProperty(obj, name, list => (list[name] = {
        subject: createTracked(value)
    }).subject);
};
const getAllTrackedProperties = (obj) => {
    ...
    return Object.keys(list)
        ...
        return definition.computed ?
            getComputedProperty(obj, key) :
            getTrackedProperty(obj, key);
    });
};
const getAllObservables = (obj) => {
    return getAllTrackedProperties(obj)
        .map(tracked => tracked.observable);
};
const subscribeAll = (obj, observer) => {
...
function subscribe(obj, propertyName, observer) {
    var _a;
    const isComputed = (_a = getTrackedPropertyDefinitionList(obj)[propertyName]) === null || _a === void 0 ? void 0 : _a.computed;
    if (isComputed) {
        ...
        return getTrackedProperty(obj, propertyName)
            .subscribe(observer);
    }
}

export { TrackedArray, TrackedComputedSubject, TrackedSubject, TrackedEntity, createTracked, tracked, trackable, isTrackableEntity, computed, getTrackedPropertyList, getTrackedProperty, getOrSetupTrackedProperty, getAllTrackedProperties, getAllObservables, subscribeAll, getComputedPropertyList, getOrSetupComputedProperty, getComputedProperty, subscribe };
